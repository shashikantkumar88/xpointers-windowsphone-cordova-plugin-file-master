...
namespace WPCordovaClassLib.Cordova.Commands
{
    ...
    public class File : BaseCommand
    {
        ...
        public void SaveImageToIsolatedStorage(string options)
        {
            string[] optionsString = JSON.JsonHelper.Deserialize<string[]>(options);
            if (optionsString.Length > 0)
            {
                string imgHttpUrl = optionsString[0];
                string fileName = Path.GetFileName(imgHttpUrl);
                using (IsolatedStorageFile iso = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    if (!iso.FileExists(fileName))
                    {
                        ...
                        webClientImg.OpenReadAsync(new Uri(imgHttpUrl), fileName);
                    }
                }
            }
        }
        ...
        void client_OpenReadCompleted(object sender, OpenReadCompletedEventArgs e)
        {
            try
            {
                String imgUrl = (string)e.UserState;
                string isoStorageLocation = imgUrl;
                using (IsolatedStorageFile iso = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    IsolatedStorageFileStream isostream = iso.CreateFile(isoStorageLocation);
